<?php
    ...
    if (isset($_GET['sector'])){
        $sector = $_GET['sector'];
        $housing = $_GET['type'];
        $query = "SELECT MEDIAN_COST FROM HOUSING.COST_ENTRIES WHERE SECTOR_ID=(SELECT ID FROM SECTOR WHERE NAME=:sectorName) AND HOUSING_ID=(SELECT ID FROM HOUSING_TYPE WHERE NAME=:housingName)";
